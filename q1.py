...
bias_file = 'AES_Sbox_bias.txt'
if os.path.isfile(bias_file):
    with open(bias_file, 'r') as fp:
        bias = fp.readlines()[0]
        bias = list(map(int, bias.split(',')))
else:
    bias = []
    for i in range(1, len(SBOX)):
        for j in range(1, len(SBOX)):
            ...
            for k in range(len(SBOX)):
                cur = bin((k & i) ^ (SBOX[k] & j)).count('1')
                if cur % 2 == 0 :
                    cnt += 1
            assert cnt % 2 == 0
            bias.append(cnt)
    with open(bias_file, 'w+') as fp:
        fp.write(','.join([str(b) for b in bias]))
